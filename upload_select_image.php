...
if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
    // Բեռնումների պանակը
    $uploadDir = UPLOAD_DIR . 'uploads/images/'; // Բեռնումների ֆոլդերը
    ...
    $imagePath = MEDIA_BASE_URL_FOR_DB . "uploads/images/" . $fileName;
    ...
    if (in_array($fileType, $allowedTypes)) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $uploadFile)) {
            echo json_encode(['status' => 'success', 'message' => 'Image uploaded successfully!', 'imagePath' => $imagePath]);
        } else {
