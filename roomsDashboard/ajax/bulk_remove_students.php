...
try {
    ...
    foreach ($input['student_ids'] as $student_id) {
        $result = $roomManager->removeStudent($input['room_id'], $student_id);
        if ($result['success']) {
            $removed_count++;
        } else {
    ...
    if ($removed_count > 0) {
        $message = $removed_count . ' students removed successfully';
        if (count($errors) > 0) {
            $message .= '. Some errors: ' . implode(', ', array_unique($errors));
        }
        echo json_encode(['success' => true, 'removed_count' => $removed_count, 'message' => $message]);
    } else {
        echo json_encode(['success' => false, 'message' => 'No students were removed. Errors: ' . implode(', ', $errors)]);
    }
